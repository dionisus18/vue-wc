...
const _tmpl$ = /*#__PURE__*/template(`<a href="#"></a>`),
      _tmpl$2 = /*#__PURE__*/template(`<div class="spinner-container"></div>`);

function readMore(props) {
  const [readMore, setReadMore] = createSignal(false);
  console.log(props);
  const LocalTemplate = createMemo(() => template(readMore() ? props?.text : props.text.slice(0, props["max-len"])));
  createEffect(() => console.log("template", LocalTemplate()));
  return [createComponent(LocalTemplate, {}), (() => {
    const _el$ = _tmpl$.cloneNode(true);

    _el$.$$click = () => setReadMore(!readMore());

    insert(_el$, () => readMore() ? props["read-less"] || "Leer Menos..." : props["read-more"] || "Leer Más...");
